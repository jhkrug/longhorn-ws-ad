...
. Restore a backing image from the backupstore, For more information, see link:../backing-image-backup[Backing Image Backup].
...
* Longhorn automatically cleans up the unused backing image files in the disks based on link:../../../references/settings#backing-image-cleanup-wait-interval[the setting `Backing Image Cleanup Wait Interval`]. But Longhorn will retain at least one file in a disk for each backing image anyway.
