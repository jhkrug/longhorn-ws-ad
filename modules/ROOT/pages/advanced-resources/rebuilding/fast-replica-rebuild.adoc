...
+
The setting enables fast replica rebuilding feature. It relies on the checksums of snapshot disk files, so setting the snapshot-data-integrity to *enable* or *fast-check* is a prerequisite. Please refer to link:../../data-integrity/snapshot-data-integrity-check[Snapshot Data Integrity Check].
