...
#2507] for details.
* Enabling the link:../references/settings#freeze-filesystem-for-snapshot[Freeze Filesystem for Snapshot] setting: Use a
kernel with version `5.17` or later to ensure that a volume crash during a filesystem freeze cannot lock up a node.
* Enabling the link:../v2-data-engine/prerequisites[V2 Data Engine]: Use a kernel with version `5.19` or later to ensure
...
Don't use a symbolic link for the extra disks. Use `mount --bind` instead of `ln -s` and make sure it's in the `fstab` file. For details, see link:../nodes-and-volumes/nodes/multidisk/#use-an-alternative-path-for-a-disk-on-the-node[the section about multiple disk support.]

== Configuring Default Disks Before and After Installation

To use a directory other than the default `/var/lib/longhorn` for storage, the `Default Data Path` setting can be changed before installing the system. For details on changing pre-installation settings, refer to link:../advanced-resources/deploy/customizing-default-settings[this section.]

The link:../nodes-and-volumes/nodes/default-disk-and-node-config[Default node/disk configuration] feature can be used to customize the default disk after installation. Customizing the default configurations for disks and nodes is useful for scaling the cluster because it eliminates the need to configure Longhorn manually for each new node if the node contains more than one disk, or if the disk configuration is different for new nodes. Remember to enable `Create default disk only on labeled node` if applicable.
...
* *Storage network*: Use a link:../advanced-resources/deploy/storage-network/#setting-storage-network[dedicated storage network] to improve IO performance and stability.
* *Longhorn disk*: Use a link:../nodes-and-volumes/nodes/multidisk/#add-a-disk[dedicated disk] for Longhorn storage instead of using the root disk.
* *Replica count*: Set the link:../references/settings/#default-replica-count[default replica count] to "2" to achieve data availability with better disk space usage or less impact to system performance. This practice is especially beneficial to data-intensive applications.
* *Storage tag*: Use link:../nodes-and-volumes/nodes/storage-tags[storage tags] to define storage tiering for data-intensive applications. For example, only high-performance disks can be used for storing performance-sensitive data.
* *Data locality*: Use `best-effort` as the default link:../high-availability/data-locality[data locality] of Longhorn StorageClasses.
+
...
+
For applications with replication capability, periodically link:../concepts/#243-deleting-snapshots[delete all types of snapshots].

* *Recurring filesystem trim*: Periodically link:../nodes-and-volumes/volumes/trim-filesystem[trim the filesystem] inside volumes to reclaim disk space.
* *Snapshot space management*: link:../snapshots-and-backups/snapshot-space-management[Configure global and volume-specific settings] to prevent unexpected disk space exhaustion.

=== Disaster Recovery

* *Recurring backups*: Create link:../snapshots-and-backups/scheduling-backups-and-snapshots/[recurring backup jobs] for mission-critical application volumes.
* *System backup*: Create periodic link:../advanced-resources/system-backup-restore/backup-longhorn-system/#create-longhorn-system-backup[system backups].

== Deploying Workloads

If you're using `ext4` as the filesystem of the volume, we recommend adding a liveness check to workloads to help automatically recover from a network-caused interruption, a node reboot, or a Docker restart. See link:../high-availability/recover-volume[this section] for details.
...
Refer to link:../references/settings/#guaranteed-instance-manager-cpu[Guaranteed Instance Manager CPU] for more details.
...
We don't recommend modifying the default StorageClass named `longhorn`, since the change of parameters might cause issues during an upgrade later. If you want to change the parameters set in the StorageClass, you can create a new StorageClass by referring to the link:../references/examples/#storageclass[StorageClass examples].
