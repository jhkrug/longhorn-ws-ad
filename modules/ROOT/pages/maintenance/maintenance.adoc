...
can control this behavior with the setting link:../../references/settings#node-drain-policy[Node Drain Policy], or
link:../../nodes-and-volumes/nodes/disks-or-nodes-eviction[evict the replica to other nodes before draining]. See <<node-drain-policy-recommendations,Node Drain Policy
...
node, Longhorn might use those replicas to speed up the rebuilding process. You can set the link:../../references/settings#replica-replenishment-wait-interval[Replica Replenishment
...
. Consider setting the setting link:../../references/settings#node-drain-policy[Node Drain Policy] to
...
See the instruction at link:../../advanced-resources/support-managed-k8s-service[Support Managed Kubernetes Service].
