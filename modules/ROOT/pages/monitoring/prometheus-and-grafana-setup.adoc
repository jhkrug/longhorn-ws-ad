...
See link:../metrics[Longhorn Metrics for Monitoring] for available metrics.
...
image::/img/screenshots/monitoring/longhorn-monitoring-system.png[images]
...
. Create PrometheusRule custom resource to define alert conditions. See more examples about Longhorn alert rules at link:../alert-rules-example[Longhorn Alert Rule Examples].
+
[,yaml]
----
 apiVersion: monitoring.coreos.com/v1
