...
* There is no existing replica using the disk, including any replicas in an error state. About how to evict replicas from disabled disks, refer to link:../disks-or-nodes-eviction/#select-disks-or-nodes-for-eviction[Select Disks or Nodes for Eviction]
