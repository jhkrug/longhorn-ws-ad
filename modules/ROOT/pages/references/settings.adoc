...
To configure Longhorn before installing it, see link:../../advanced-resources/deploy/customizing-default-settings[this section] for details.
...
Each option has benefits and drawbacks. See link:../../maintenance/maintenance/#node-drain-policy-recommendations[Node Drain Policy
...
* Workload pods with _cluster network_ RWX volumes. The setting does not apply to such pods because the Longhorn Share Manager, which provides the RWX NFS service, has its own resilience mechanism. This mechanism ensures availability until the volume is reattached without relying on the pod lifecycle to trigger volume reattachment. The setting does apply, however, to workload pods with _storage network_ RWX volumes. For more information, see link:../../nodes-and-volumes/volumes/rwx-volumes[ReadWriteMany (RWX) Volume] and link:../../advanced-resources/deploy/storage-network#limitation[Storage Network].
====
...
This option is useful if you want to scale the cluster but don't want to use storage on the new nodes, or if you want to link:../../nodes-and-volumes/nodes/default-disk-and-node-config[customize disks for Longhorn nodes].
...
Allows engine controller and engine replica to disable revision counter file update for every data write. This improves the data path performance. See link:../../advanced-resources/deploy/revision_counter[Revision Counter] for details.
...
Setting that allows you to enable the V2 Data Engine, which is based on the Storage Performance Development Kit (SPDK). The V2 Data Engine is a preview feature and should not be used in production environments. For more information, see link:../../v2-data-engine[V2 Data Engine (Preview Feature)].
...
Endpoint used to access a backupstore.   Longhorn supports AWS S3, Azure, GCP, CIFS and NFS.  See link:../../snapshots-and-backups/backup-and-restore/set-backup-target[Setting a Backup Target] for details.
...
The Kubernetes secret associated with the backup target. See link:../../snapshots-and-backups/backup-and-restore/set-backup-target[Setting a Backup Target] for details.
...
The interval in seconds to poll the backup store for updating volumes' *Last Backup* field. Set to 0 to disable the polling. See link:../../snapshots-and-backups/setup-disaster-recovery-volumes[Setting up Disaster Recovery Volumes] for details.

For more information on how the backupstore poll interval affects the recovery time objective and recovery point objective, refer to the link:../../concepts/#34-backupstore-update-intervals-rto-and-rpo[concepts section.]
...
NOTE: It's recommended to disable this setting when using Longhorn in the production environment. See link:../../best-practices/[Best Practices] for details.
...
See link:../../nodes-and-volumes/nodes/multidisk/#configuration[Multiple Disks Support] for details.
...
See link:../../advanced-resources/deploy/taint-toleration[Taint Toleration] for details.
...
See link:../../advanced-resources/deploy/priority-class[Priority Class] for details.
...
Longhorn system contains user deployed components (e.g, Longhorn manager, Longhorn driver, Longhorn UI) and system managed components (e.g, instance manager, engine image, CSI driver, etc.)
You need to set node selector for both of them. This setting only sets node selector for system managed components. Follow the instruction at link:../../advanced-resources/deploy/node-selector[Node Selector] to change node selector.
...
See link:../../high-availability/k8s-cluster-autoscaler[Kubernetes Cluster Autoscaler Support] for details.
...
See link:../../advanced-resources/deploy/storage-network[Storage Network] for details.
...
For more information, see link:../../advanced-resources/deploy/storage-network[Storage Network].
...
See link:../../nodes-and-volumes/volumes/trim-filesystem[Trim Filesystem] for details.
