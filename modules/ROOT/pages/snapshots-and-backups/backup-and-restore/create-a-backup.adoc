...
Backups in Longhorn are objects in an off-cluster backupstore. A backup of a snapshot is copied to the backupstore, and the endpoint to access the backupstore is the backup target. For more information, see link:../../../concepts/#31-how-backups-work[this section.]

____
*Prerequisite:* A backup target must be set up. For more information, see link:../set-backup-target[Set the BackupTarget]. If the BackupTarget has not been set, you'll be presented with an error.
____
...
For information about restoring a volume from a snapshot, see link:../restore-from-a-backup[Restore from a Backup].
